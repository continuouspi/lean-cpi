...
| symm  {Γ} {b y} {F G : concretion ω Γ b y} : equiv F G → equiv G F
...
| parallel_symm
    {Γ} {b y} {F : concretion ω Γ b y} {A : species ω Γ}
  : equiv (F |₁ A) (A |₂ F)
| parallel_assoc₁
...
| ν_swap
    {Γ} {b y} (M N : affinity)
    {F : concretion ω (context.extend N.arity (context.extend M.arity Γ)) b y}
  : equiv (ν'(M)ν'(N) F) (ν'(N)ν'(M) rename name.swap F)

| apply_parallel
    {Γ} {b y} {bs : vector (name Γ) b}
    {A : species ω Γ} {B : species ω (context.extend y Γ)}
  : equiv (#(bs; y) (species.rename name.extend A |ₛ B)) (A |₂ #(bs; y) B)

instance {Γ} {b y} : is_equiv (concretion ω Γ b y) equiv :=
  { refl := @equiv.refl _ Γ b y, symm := @equiv.symm _ Γ b y, trans := @equiv.trans _ Γ b y }
instance {Γ} {b y} : is_refl (concretion ω Γ b y) equiv := ⟨ λ _, equiv.refl ⟩
instance {Γ} {b y} : setoid (concretion ω Γ b y) :=
