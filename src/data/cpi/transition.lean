...
namespace production
  def equiv.refl {Γ : context ω} : ∀ {k : kind} (E : production Γ k), equiv E E
  | ._ (species A) := equiv.species (refl A)
  | ._ (concretion F) := equiv.concretion (refl F)

  def equiv.symm {Γ : context ω} : ∀ {k : kind} (E F : production Γ k), equiv E F → equiv F E
  | ._ ._ ._ (equiv.species eq) := equiv.species (symm eq)
  | ._ ._ ._ (equiv.concretion eq) := equiv.concretion (symm eq)

  def equiv.trans {Γ : context ω} : ∀ {k : kind} (E F G : production Γ k), equiv E F → equiv F G → equiv E G
  ...
  instance {Γ : context ω} {k} : is_equiv (production Γ k) equiv :=
    { refl := equiv.refl, symm := equiv.symm, trans := equiv.trans }
  instance {Γ : context ω} {k} : is_refl (production Γ k) equiv := ⟨ equiv.refl ⟩
  instance {Γ : context ω} {k} : setoid (production Γ k) :=
    ⟨ equiv, ⟨ equiv.refl, equiv.symm, equiv.trans ⟩ ⟩
  instance setoid.is_equiv {Γ : context ω} {k} : is_equiv (production Γ k) has_equiv.equiv :=
    production.is_equiv
end production

/-- A transition from a species to some production of a given kind. -/
inductive label : context ω → kind → Type
...
def label.replace {Γ Δ : context ω} (ρ : name Γ → name Δ) : ∀ {k}, label Γ k → label Δ k
| ._ #a := # (ρ a)
| ._ τ@'k := τ@'k
| ._ τ⟨ a, b ⟩ := τ⟨ ρ a, ρ b ⟩

inductive transition : Π {Γ : context ω} {k}, species Γ → label Γ k → production Γ k → Type
...
constant φ_convert :
  ∀ {Γ : context ω} {A : species Γ} (B : species Γ) {k} {l : label Γ k} {E : production Γ k}
