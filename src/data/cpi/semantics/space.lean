...
/-- A vector-space representation of processes, mapping prime species into their
    concentrations. -/
def process_space (ℂ ℍ : Type) (ω Γ : context) [add_monoid ℂ] := fin_fn (prime_species' ℍ ω Γ) ℂ
...
noncomputable instance process_space.add_comm_monoid {ω Γ}
  : add_comm_monoid (process_space ℂ ℍ ω Γ)
  := fin_fn.add_comm_monoid _ ℂ

noncomputable instance process_space.has_sub {ω Γ}
  : has_sub (process_space ℂ ℍ ω Γ)
  := fin_fn.has_sub _ ℂ

noncomputable instance process_space.distrib_mul_action {ω Γ}
...
@[simp]
lemma to_process_space.nil {Γ} : @to_process_space ℂ ℍ ω _ Γ ⟦nil⟧ = 0 := begin
...
/-- The vector space (A, E, a)→ℍ relating transitions from A to E with label #a.
  -/
def interaction_space (ℂ ℍ : Type) (ω Γ : context) [add_monoid ℂ] : Type
  := fin_fn
      ( species' ℍ ω Γ
...
noncomputable instance interaction_space.has_sub {ω Γ}
  : has_sub (interaction_space ℂ ℍ ω Γ)
  := fin_fn.has_sub _ ℂ

noncomputable instance interaction_space.distrib_mul_action {ω Γ}
...
axiom process.from_inverse {Γ} :
  function.left_inverse process.to_space' (@process.from_space ℂ ℍ ω _ Γ)
